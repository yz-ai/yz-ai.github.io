...
Bu bölümde yöntem olarak, açgözlü (ε-greedy) kullanılacaktır çünkü, genellikle tahmini maksimum eylem değerine sahip olanı seçtikleri içindir. Yani, tüm aç gözlü olmayan (non-greedy) eylemlere minimum seçim olasılığı verilir, $\frac{\epsilon}{|A(s)|}$, ve olasılığın $(1-\epsilon)+\frac{\epsilon}{|A(s)|}$ büyüklüğü, açgözlü (greedy) eyleme verilir. $\epsilon$-açgözlü politikalar, tüm durumlar ve eylemler için bazı $\epsilon > 0$ olan politikalar olarak tanımlanan  $\pi(a|s) \geq \frac{\epsilon}{|A(s)|}$, $\epsilon-soft$ politikaların örnekleridir. $\epsilon-soft$ politikaların içinden, $\epsilon-greedy$ politikalar açgözlülüğe en yakın olanlarıdır.
...
![Şekil 5.3: Ağırlıklandırılmış önem örneklemesi, politika dışı bölümlerden alınan tek bir blackjack durumunun daha düşük hata tahminleri üretir.]({{ site.url }}/assets/sekil-53.png)
