...
\begin{align}
    G_{t:t+n}\doteq R_{t+1} +\gamma R_{t+2} + ... +\gamma^{n-1}R_{t+n}+ \gamma^{n}V_{t+n-1}(S_{t+n}) , \qquad (7.1)
\end{align}
...
\begin{align}
    V_{t+n}(S_{t})\doteq V_{t+n-1}(S_{t})+\alpha [G_{t:t+n-1} - V_{t+n-1}(S_{t})] , \quad 0 \leq t < T  \qquad (7.2)
\end{align}
...
$$
    \max_{s}\mid E_{\pi} \left[G_{t:t+n} \mid S_{t} =s \right] -v _{t}(s) \mid \leq \gamma ^{n} \max_{s} \mid V_{t+n-1}(s)-v_{\pi}(s)\mid ,  \qquad (7.3)
$$
...
__*Şekil 7.3 :*__ *Durum-eylem değerleri için $$n$$-adım yöntemlerinin spektrum yedek diyagramları. Bunlar, Sarsa(0)’ın bir adım güncellemesinden Monte Carlo yönteminin sonlandırma güncellemesine kadar uzanmaktadır. Bunlar arasında, $$n$$-adım güncellemeleri, gerçek ödüllerin $$n$$-adımlarına ve sonraki tüm durum-eylem çiftinin tahmini değerine bağlı olarak güncellenir. En sağdaki  Beklenen $$n$$-adım Sarsa için diyagram.*
...
\begin{align}
    G_{t:t+n} \doteq R_{t+1}+\gamma R_{t+2}+...+ \gamma^{n-1} R_{t+n} + \gamma^{n} Q_{t+n-1} (S_{t+n}, A_{t+n}) ,n\geq 1, 0\leq t < T-n  \qquad (7.4)
\end{align}
...
\begin{equation}
\label{eq:75}
    Q_{t+n}(S_{t},A_{t})\doteq Q_{t+n-1}(S_{t},A_{t})+\alpha [G_{t:t+n} - Q_{t+n-1}(S_{t},A_{t})],   0 \leq t < T \qquad (7.5)
\end{equation}
...
$$
    \label{eq:77}
    G_{t:t+n}\doteq R_{t+1} +...+\gamma^{n-1}R_{t+n}+ \gamma^{n}\overline{V}_{t+n-1}(S_{t+n}),   t+n<T \qquad (7.7)
$$
...
$$
\label{eq:78} 
\overline{V}_{t}(s)\doteq \sum_{a}\pi (a|s) Q_{t}(s,a), \qquad (7.8)
$$ tüm  $$s$$ $$\in S$$ için
...
\begin{equation}
\label{eq:79}
    V_{t+n}(S_{t})\doteq V_{t+n-1}(S_{t})+\alpha p_{t:t+n-1}[G_{t:t+n-1} - V_{t+n-1}(S_{t})] ,0\leq t< T \qquad (7.9)
\end{equation}

$$p_{t:t+n-1}$$ : Önem örnekleme oranı.

\begin{align}
    \rho_{t:h} \ \dot{=} \displaystyle\prod\limits_{k=t}^{min(h,T-1)}\dfrac{\pi(A_k|S_k)}{b(A_k|S_k)} \qquad (7.10)
\end{align}
...
$$\begin{align*}
G_{t:h}=R_{t+1}+ \gamma G_{t+1:h} \qquad (7.12)
\end{align*}$$ 
...
\begin{equation}
    \label{eq:7.13} 
    G_{t:h} \dot{=} \rho_{t}(R_{t+1} + \gamma G_{t+1:h})+(1-\rho_t) V_{h-1}(S_t), \qquad t<h<T \qquad (7.13)
\end{equation}
...
\begin{equation}
\label{eq:7.16}
   G_{t:t+n} \dot{=} R_{t+1} + \gamma \displaystyle \sum_{a \neq A_{t+1}} \pi(a \mid S_{t+1})Q_{t+n-1}(S_{t+1},a) + \gamma \pi (A_{t+1} \mid S_{t+1}) G_{t+1:t+n} \qquad (7.16)
\end{equation}
...
Daha sonra, eğer her zaman örneklemeyi seçtiyse, kişi Sarsa'yı elde edecekti, oysa hiç biri örneklemeyi seçmezse, ağaç-yedekleme algoritmasını alacaktı. Şekildeki son diyagramın önerdiği gibi, başka birçok olasılık da olacaktır.

![Şekil 7.5]({{ site.url }}/assets/images/RL-sutton-ozet/sekil-75.png)

__*Şekil 7.5 :*__ *Bu bölümde (4 aşamalı durum) şimdiye kadar ele alınan üç adımlı n adımlı eylem değeri güncellemelerinin yedek diyagramları ve bunların tümünü birleştiren dördüncü bir güncelleme türünün yedek diyagramı. ‘Ρ’lar, politika dışı vakada önemlilik örneklemenin gerekli olduğu yarı geçişleri belirtir. Dördüncü güncelleme türü, örneklemenin $$\sigma_{t}=1$$ olup olmadığına $$\sigma_{t}=0$$ göre durum bazında seçim yaparak diğerlerini birleştirir. $$\sigma_{t} \epsilon [0,1]$$*
